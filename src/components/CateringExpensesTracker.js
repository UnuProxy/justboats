...
class CateringExpensesTracker extends React.Component {
  ...
  loadOrders = async () => {
    try {
      ...
      // Get orders from firestore - without the orderBy clause that requires an index
      const ordersRef = collection(db, 'orders');
      const q = query(ordersRef, where('status', 'in', ['pending', 'confirmed', 'delivered']));
      ...
      this.setState({
        ...
        // Calculate expense summary by category after state update
        if (firstId) {
          this.calculateExpenseSummaryByCategory();
        }
      });
  ...
  calculateExpenseSummaryByCategory = () => {
  ...
  handleAddExpense = async () => {
    ...
    try {
      ...
      this.setState(prevState => {
        ...
        // Update expense category summary
        this.calculateExpenseSummaryByCategory();
      });
  ...
  handleDeleteExpense = async (expenseId) => {
    ...
    if (!selectedOrderId || !expenseId) return;
    ...
    try {
      ...
      const expenseRef = doc(db, 'orders', selectedOrderId, 'expenses', expenseId);
  ...
  render() {
    const { orders, loading, error, selectedOrderId, newExpense, activeTab, expenseSummaryByCategory } = this.state;
    const selectedOrder = this.getSelectedOrder();
                        ...
                        <div className="mb-6">
                          ...
                          <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
                              ...
                              <span className="text-lg font-bold">{this.formatCurrency(selectedOrder.totalExpenses)}</span>
                            </div>
                          </div>
